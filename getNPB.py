...
# including 1950~2016 data of both central and pacific league
...
def getNpbData(year, league): # year is 1950 ~ 2016 / league is "central" or "pacific"
	...
	# team, all, win, lose, draw, winprob, batting ave, bats, get points, hits,
	# double, triple, HR, RBI, steal, protection ratio, save, whole pitch,
	# strikeouts, lost points

	table_1_array = []
	table_2_array = []
	table_3_array = []

	table_1_tr = tables[0].find('table').find_all('tr')
	for i in range(len(table_1_tr)):
		table_1_td = table_1_tr[i].find_all('td')
		td_array = []
		for n in range(len(table_1_td)):
